...
const WhatsAppPreview: React.FC<WhatsAppPreviewProps> = ({
  ...
  const { summaryText, imageUrl, imageSuggestion, actionText } =
  ...
  const commonImageContainerClass =
    "my-2 mx-auto max-h-40 flex items-center justify-center";
  ...
  if (imageUrl) {
    if (imageUrl.startsWith("http") || imageUrl.startsWith("data:")) {
      ...
      imageElement = (
        <div
          className={`${commonImageContainerClass} py-1 text-5xl`}
          role="img"
          aria-label="icon"
        >
          {imageUrl}
      ...
      imageElement = (
        <div
          className={`${commonImageContainerClass} py-1 text-gray-500 text-xs italic flex-col`}
        >
          <span className="opacity-70 text-4xl mb-1">
    ...
    <div className="w-full max-w-xs sm:max-w-sm mx-auto bg-white rounded-xl shadow-xl overflow-hidden my-4 border border-gray-200">
      ...
      <div className="bg-gray-100 p-2.5 flex items-center justify-between border-b border-gray-200">
        <div className="flex items-center">
          <span
            className="text-gray-600 hover:text-gray-800 cursor-pointer text-lg mr-1.5"
            role="img"
            aria-label="Back"
          >
            ‹
      ...
      <div className="bg-accent p-3.5 min-h-[18rem] sm:min-h-[20rem] overflow-y-auto flex flex-col space-y-2.5">
        <div className="text-center my-2">
          <span className="bg-gray-200/90 text-gray-600 text-[11px] px-2 py-0.5 rounded-md shadow-sm">
            TODAY
          </span>
        </div>
          ...
          >
            ...
            <div
              className={`absolute bottom-0 w-2 h-2 ${
                isUserMessage
                  ? "right-[-7px] bg-primary-light transform skew-y-[-20deg] shadow-sm"
                  : "left-[-7px] bg-white transform skew-y-[20deg] shadow-sm"
              }`}
              style={
                isUserMessage
            ...
            {actionText && (
              ...
              >
                {actionText}{" "}
                {/* Icon removed for cleaner look, like WhatsApp links */}
              </button>
            )}
            <div
              className={`text-[11px] mt-1 ${
                isUserMessage
                  ? "text-right text-green-700/80"
                  : "text-right text-gray-500/80"
              }`}
            >
              {time}
      ...
      <div className="bg-gray-100 p-2 flex items-center border-t border-gray-200 space-x-2">
        <span
          className="text-gray-500 text-2xl p-1.5 hover:bg-gray-200 rounded-full cursor-pointer"
          role="img"
          aria-label="Emoji"
        >
          😀
        ...
        />
        <span
          className="text-gray-500 text-xl p-1.5 hover:bg-gray-200 rounded-full cursor-pointer"
          role="img"
          aria-label="Attach"
        >
          📎
        ...
        <button
          className="bg-primary text-white w-10 h-10 rounded-full flex items-center justify-center shadow-md hover:bg-primary-dark transition-colors"
